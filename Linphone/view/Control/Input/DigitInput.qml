...
Control.TextField {
	id: mainItem
	property int inputSize: 100 * mainWindow.dp
	...
	background: Item {
		...
		Rectangle {
			id: background
			border.width: Math.max(mainWindow.dp, 1)
		...
		Rectangle {
			...
			height : Math.max(0.5, 1 * mainWindow.dp)
			width: mainItem.inputSize * 0.67
			anchors.horizontalCenter: parent.horizontalCenter
			anchors.bottom: parent.bottom
			anchors.bottomMargin: (mainItem.inputSize / 8) * mainWindow.dp
		}
	}
}
