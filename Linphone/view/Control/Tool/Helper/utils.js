...
function openWindow(window, parent, options, fullscreen) {
	...
	if (runOnWindows()) {
		object.show(
					); // Needed for Windows : Show the window in all case. Allow to graphically locate the window before going to fullscreen.
		if (fullscreen)
			object.showFullScreen();// Should be equivalent to changing visibility
	} else if (fullscreen)
		object.showFullScreen();
	// Should be equivalent to changing visibility
	else
		object.show();
	return object;
}
...
function pointIsInItem(source, target, point) {
	...
	return (point.x >= target.x && point.y >= target.y && point.x < target.x + target.width && point.y < target.y
			+ target.height);
}
...
function escapeQuotes(str) {
	return str != null ? str.replace(/([^'\\]*(?:\\.[^'\\]*)*)'/g, '$1\\\'') : '';
}
...
function equalObject(a, b) {
	...
	if (a == b)
		return true; // operator could be performed
	for (var i in a) {
...
function updatePosition(scrollItem, list) {
	...
	if (item && (list.expanded || list.expanded == undefined)) {
		...
		if (list != scrollItem)
			topItemPos += list.y;
		if (list.headerHeight)
			topItemPos += list.headerHeight;
...
function getSizeWithScreenRatio(size) {
	return (size == 0) ? 0 : size > 0 ? Math.max(Math.round(size * Linphone.DefaultStyle.dp), 1) : Math.min(Math.round(
																												size * Linphone.DefaultStyle.dp), -1);
}
