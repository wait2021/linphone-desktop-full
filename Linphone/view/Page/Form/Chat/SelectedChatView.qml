...
FocusScope {
	...
	property bool showEncryptedInfo: currentAccount && currentAccount.core.limeServerUrl !== ""
									 && currentAccount.core.conferenceFactoryAddress !== ""
	enum PanelType {
		MessageReactions,
		SharedFiles,
		Medias,
	...
	onActiveFocusChanged: if (activeFocus) {
							  if (chatMessagesListView.lastItemVisible)
	...
	onOneOneCall: {
		if (contact)
			...
			UtilsCpp.createCall(mainItem.chat?.core.peerAddress, {
									'localVideoEnabled': video
								});
	}

	onGroupCall: {
		mainWindow.showConfirmationLambdaPopup("", qsTr("chat_view_group_call_toast_message"), "", function (confirmed) {
			if (confirmed) {
	...
	RowLayout {
		...
		MainRightPanel {
				...
				GridLayout {
					...
					RowLayout {
						...
						ColumnLayout {
							Text {
								Layout.fillWidth: true
								Component.onCompleted: console.log(text, "width", width, "implicitWidth", implicitWidth, "advance", advance,
																   contentWidth)
						...
						RoundButton {
							...
							Connections {
								...
								function onVisibleChanged() {
									searchInHistoryButton.checked = searchBarLayout.visible;
								}
							}
						}
						RoundButton {
							...
							Connections {
								...
								function onVisibleChanged() {
									if (!detailsPanel.visible)
										detailsPanelButton.checked = false;
								}
							}
						}
					}
				...
				RowLayout {
					...
					onVisibleChanged: {
						if (!visible)
							...
							chatMessagesSearchBar.forceActiveFocus();
					}
			...
			content: Control.SplitView {
						...
						contentItem: ColumnLayout {
							...
							ColumnLayout {
								...
								Text {
									Layout.fillWidth: true
									//: Reply to %1
									text: mainItem.replyingToMessage ? (mainItem.chatMessage ? qsTr("reply_to_label").arg(UtilsCpp.boldTextPart(
																															  mainItem.chatMessage.core.fromName, mainItem.chatMessage.core.fromName)) : "") : qsTr(
				...
				ChatDroppableTextArea {
					...
					onChatChanged: {
						if (chat)
							messageSender.text = mainItem.chat.core.sendingText;
					}
					onTextChanged: {
						if (text !== "") {
					...
					onSendMessage: {
						...
						if (mainItem.replyingToMessage) {
							...
							mainItem.chat.core.lSendMessage(text, filesContents);
						contents.clear();
					}
					onDropped: files => {
		...
		Control.Control {
			...
			contentItem: Loader {
				...
				sourceComponent: panelType === SelectedChatView.PanelType.EphemeralSettings ? ephemeralSettingsComponent :
																							  panelType === SelectedChatView.PanelType.MessageReactions ? messageReactionsComponent :
																																						  panelType === SelectedChatView.PanelType.ImdnStatus
																																						  ? messageImdnStatusComponent : panelType
																																							=== SelectedChatView.PanelType.SharedFiles || panelType
																																							=== SelectedChatView.PanelType.Medias ? sharedFilesComponent : panelType
																																																	=== SelectedChatView.PanelType.ForwardToList ? forwardToListsComponent :
																																																												   panelType
																																																												   === SelectedChatView.PanelType.ManageParticipants
																																																												   ? manageParticipantsComponent : infoComponent
			...
			Component {
				...
				ConversationInfos {
					...
					onShowSharedFilesRequested: showMedias => {
													contentLoader.panelType = showMedias ? SelectedChatView.PanelType.Medias :
																						   SelectedChatView.PanelType.SharedFiles;
												}
			...
			Component {
				...
				MessageSharedFilesInfos {
					...
					filter: contentLoader.panelType === SelectedChatView.PanelType.Medias
							? ChatMessageFileProxy.FilterContentType.Medias : ChatMessageFileProxy.FilterContentType.Documents
					onGoBackRequested: {
						// detailsPanel.visible = false
						contentLoader.panelType = SelectedChatView.PanelType.None;
					}
						...
						Flickable {
							...
							ColumnLayout {
								...
								AllContactListView {
									...
									property var chatForSelectedAddressObj: selectedFriend ? UtilsCpp.getChatForAddress(
																								 selectedFriend.core.defaultAddress) : null
