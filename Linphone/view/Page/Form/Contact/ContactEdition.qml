		...
		Button {
			...
			onClicked: {
				if (contact.core.isSaved)
					...
					showConfirmationLambdaPopup("", qsTr("contact_editor_dialog_cancel_change_message"), "", function (confirmed) {
						if (confirmed) {
							mainItem.contact.core.undo();
							mainItem.closeEdition('');
						}
			...
			ColumnLayout {
					...
					contentItem: TextField {
						...
						Keys.onPressed: event => {
											if (event.key == Qt.Key_Down) {
												(addressesList.count > 0 ? addressesList.itemAt(0) : newAddressTextField).forceActiveFocus(
													Qt.TabFocusReason);
												event.accepted = true;
											}
										}
						...
						contentItem: RowLayout {
							...
							function updateFocus(event) {
								if (event.key == Qt.Key_Up) {
									...
									(index + 1 < addressesList.count ? addressesList.itemAt(index + 1) : newAddressTextField).forceActiveFocus(
												Qt.TabFocusReason);
									event.accepted = true;
							...
							TextField {
								...
								onEditingFinished: {
									...
									if (text.length != 0)
										mainItem.contact.core.setAddressAt(index, label, text);
								}
				...
				FormItemLayout {
					...
					contentItem: TextField {
						...
						Keys.onPressed: event => {
											if (event.key == Qt.Key_Up) {
												(addressesList.count > 0 ? addressesList.itemAt(addressesList.count - 1) : jobTextField).forceActiveFocus(
													Qt.BacktabFocusReason);
												event.accepted = true;
											} else if (event.key == Qt.Key_Down) {
												(phoneNumberList.count > 0 ? phoneNumberList.itemAt(0) : phoneNumberInputTextField).forceActiveFocus(
													Qt.TabFocusReason);
												event.accepted = true;
											}
										}
				...
				Repeater {
					...
					delegate: FormItemLayout {
						...
						contentItem: RowLayout {
							...
							function updateFocus(event) {
								if (event.key == Qt.Key_Up) {
									(index - 1 >= 0 ? phoneNumberList.itemAt(index - 1) : newAddressTextField).forceActiveFocus(
												Qt.BacktabFocusReason);
									event.accepted = true;
								} else if (event.key == Qt.Key_Down) {
									(index + 1 < phoneNumberList.count ? phoneNumberList.itemAt(index + 1) :
																		 phoneNumberInputTextField).forceActiveFocus(Qt.TabFocusReason);
									event.accepted = true;
								}
							}
							TextField {
								...
								onEditingFinished: {
									//: "Téléphone"
									if (text.length != 0)
										mainItem.contact.core.setPhoneNumberAt(index, qsTr("phone"), text);
								}
				...
				FormItemLayout {
					...
					contentItem: TextField {
						...
						Keys.onPressed: event => {
											if (event.key == Qt.Key_Up) {
												(phoneNumberList.count > 0 ? phoneNumberList.itemAt(phoneNumberList.count - 1) :
																			 newAddressTextField).forceActiveFocus(Qt.BacktabFocusReason);
												event.accepted = true;
											} else if (event.key == Qt.Key_Down) {
												(contactLayoutItem.button.enabled ? contactLayoutItem.button : givenNameEdit).forceActiveFocus(
													Qt.TabFocusReason);
												event.accepted = true;
											}
										}
						onEditingFinished: {
							if (text.length != 0)
								mainItem.contact.core.appendPhoneNumber(phoneNumberInput.label, text);
				...
				TemporaryText {
					...
					onTextChanged: if (addressesErrorText.text.length > 0)
