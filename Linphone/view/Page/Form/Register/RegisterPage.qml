...
LoginLayout {
	...
	Connections {
		...
		function onErrorInField(field, errorMessage) {
			...
			if (field == "username")
				usernameItem.errorMessage = errorMessage;
			else if (field == "password")
				passwordItem.errorMessage = errorMessage;
			else if (field == "phone")
				phoneNumberInput.errorMessage = errorMessage;
			else if (field == "email")
				emailItem.errorMessage = errorMessage;
			else
	...
	titleContent: [
		...
		RowLayout {
			...
			Layout.rightMargin: Math.max(Utils.getSizeWithScreenRatio(10), Utils.getSizeWithScreenRatio(51 - ((51 / (
																												   DefaultStyle.defaultWidth - mainWindow.minimumWidth)) * (DefaultStyle.defaultWidth
																																											- mainWindow.width))))
			Text {
	...
	centerContent: [
		ColumnLayout {
			...
			TabBar {
				...
				Layout.rightMargin: Math.max(Utils.getSizeWithScreenRatio(5), Utils.getSizeWithScreenRatio(127 - ((127 / (
																													   DefaultStyle.defaultWidth - mainWindow.minimumWidth)) * (DefaultStyle.defaultWidth
																																												- mainWindow.width))))
			...
			Flickable {
				...
				ColumnLayout {
						...
						RowLayout {
							FormItemLayout {
								...
								contentItem: TextField {
									id: usernameInput
									backgroundBorderColor: usernameItem.errorMessage.length > 0 ? DefaultStyle.danger_500_main :
							...
							RowLayout {
								...
								FormItemLayout {
									...
									contentItem: TextField {
										...
										hidden: true
										Layout.preferredWidth: Utils.getSizeWithScreenRatio(346)
										backgroundBorderColor: passwordItem.errorMessage.length > 0 ? DefaultStyle.danger_500_main :
																									  DefaultStyle.grey_200
										//: "%1 mandatory"
								...
								FormItemLayout {
									...
									contentItem: TextField {
										...
										backgroundBorderColor: passwordItem.errorMessage.length > 0 ? DefaultStyle.danger_500_main :
																									  DefaultStyle.grey_200
										//: "%1 mandatory"
										Accessible.name: qsTr("mandatory_field_accessible_name").arg(qsTr(
					...
					RowLayout {
						...
						.arg(("<a href='%1'><font color='DefaultStyle.main2_600'>%2</font></a>").arg(ConstantsCpp.CguUrl).arg(qsTr(
						"assistant_dialog_general_terms_label")))
						//: "politique de confidentialité"
						.arg(("<a href='%1'><font color='DefaultStyle.main2_600'>%2</font></a>").arg(ConstantsCpp.PrivatePolicyUrl).arg(
						qsTr("assistant_dialog_privacy_policy_label")))
						CheckBox {
							id: termsCheckBox
						...
						onClicked: {
							if (usernameInput.text.length === 0) {
								...
								if (bar.currentIndex === 0)
									RegisterPageCpp.registerNewAccount(usernameInput.text, pwdInput.text, "", phoneNumberInput.countryCode,
