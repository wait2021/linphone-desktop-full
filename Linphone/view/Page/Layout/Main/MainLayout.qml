...
Item {
	...
	Item {
		...
		Popup {
			...
			visible: currentCall && currentCall.core.state != LinphoneEnums.CallState.Idle && currentCall.core.state
					 != LinphoneEnums.CallState.IncomingReceived && currentCall.core.state
					 != LinphoneEnums.CallState.PushIncomingReceived
			...
			contentItem: MediumButton {
				...
				text: currentCallNotif.currentCall ? currentCallNotif.currentCall.core.conference ? ("Réunion en cours : ")
																									+ currentCallNotif.currentCall.core.conference.core.subject : (("Appel en cours : ")
																																								   + currentCallNotif.remoteName) : "appel en cours"
				onClicked: {
			...
			ColumnLayout {
				...
				RowLayout {
					...
					SearchBar {
						...
						KeyNavigation.down:
							contactList //contactLoader.item?.count > 0 || !contactLoader.item?.footerItem? contactLoader.item : contactLoader.item?.footerItem
						KeyNavigation.up:
							contactList //contactLoader.item?.footerItem ? contactLoader.item?.footerItem : contactLoader.item
						Keys.onPressed: event => {
											...
											if (event.key === Qt.Key_Enter || event.key === Qt.Key_Return && firstContact.searchResultItem) {
												console.log("Enter pressed from magic search bar, calling",
															firstContact.searchResultItem.core.defaultAddress);
							...
							popup.contentItem: FocusScope {
								...
								ColumnLayout {
									...
									IconLabelButton {
										...
										onClicked: openAccountSettings(accountProxy.defaultAccount ? accountProxy.defaultAccount :
									...
									IconLabelButton {
										...
										onClicked: {
											settingsMenuButton.popup.close();
											//: "Quitter %1 ?"
											UtilsCpp.getMainWindow().showConfirmationLambdaPopup("", qsTr("quit_app_question").arg(applicationName), "", function (
												confirmed) {
												if (confirmed) {
