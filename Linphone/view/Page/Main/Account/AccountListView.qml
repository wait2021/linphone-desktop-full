...
ColumnLayout {
	...
	Component {
		...
		Contact {
			...
			KeyNavigation.up: visibleChildren.length != 0 ? getPreviousItem(index) : null
			KeyNavigation.down: visibleChildren.length != 0 ? getNextItem(index) : null
		}
	}
	...
	IconLabelButton {
		...
		KeyNavigation.up: visibleChildren.length != 0 ? getPreviousItem(AppCpp.accounts.getCount()) : null
		KeyNavigation.down: visibleChildren.length != 0 ? getNextItem(AppCpp.accounts.getCount()) : null
	}
}
