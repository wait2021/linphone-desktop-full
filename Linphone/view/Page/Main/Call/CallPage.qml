	...
	*/
	...
	onSelectedRowHistoryGuiChanged: {
		if (selectedRowHistoryGui)
			rightPanelStackView.replace(contactDetailComp, Control.StackView.Immediate);
		else
			rightPanelStackView.replace(emptySelection, Control.StackView.Immediate);
	}
	...
	showDefaultItem: listStackView.currentItem && listStackView.currentItem.objectName == "historyListItem"
					 && listStackView.currentItem.listView.count === 0 || false

	function goToNewCall() {
		if (listStackView.currentItem && listStackView.currentItem.objectName != "newCallItem")
			listStackView.push(newCallItem);
	}
	function goToCallHistory() {
		if (listStackView.currentItem && listStackView.currentItem.objectName != "historyListItem")
			...
			ColumnLayout {
				...
				Rectangle {
					...
					RowLayout {
						...
						Text {
							text: qsTr("call_forward_to_address_info") + (SettingsCpp.callForwardToAddress == 'voicemail' ? qsTr(
																																"call_forward_to_address_info_voicemail") : SettingsCpp.callForwardToAddress)
			...
			CallHistoryLayout {
					...
					popup.contentItem: FocusScope {
						...
						ColumnLayout {
							...
							IconLabelButton {
								...
								KeyNavigation.up: visibleChildren.length != 0 ? detailOptions.getPreviousItem(0) : null
								KeyNavigation.down: visibleChildren.length != 0 ? detailOptions.getNextItem(0) : null
							}
							IconLabelButton {
								...
								onClicked: {
									detailOptions.close();
									var success = UtilsCpp.copyToClipboard(mainItem.selectedRowHistoryGui
									...
									if (success)
										UtilsCpp.showInformationPopup(
													...
													qsTr("sip_address_copied_to_clipboard_message"), true);
									else
										UtilsCpp.showInformationPopup(qsTr("information_popup_error_title"),
																	  //: "Erreur lors de la copie de l'adresse"
																	  qsTr("sip_address_copy_to_clipboard_error"), false);
								}
								KeyNavigation.up: visibleChildren.length != 0 ? detailOptions.getPreviousItem(1) : null
								KeyNavigation.down: visibleChildren.length != 0 ? detailOptions.getNextItem(1) : null
							}
							// IconLabelButton {
							// 	background: Item {}
							// 	enabled: false
							...
							IconLabelButton {
								...
								KeyNavigation.up: visibleChildren.length != 0 ? detailOptions.getPreviousItem(2) : null
								KeyNavigation.down: visibleChildren.length != 0 ? detailOptions.getNextItem(2) : null
							}
						}
					}
				}
				...
				detailContent: Item {
					...
					RoundedPane {
						...
						width: parent.width
						height: detailContentStack.currentIndex === 0 ? Math.min(parent.height, detailListView.contentHeight)
																		+ topPadding + bottomPadding : parent.height
										...
										RowLayout {
											...
											ColumnLayout {
												...
												RowLayout {
													EffectImage {
														...
														imageSource: modelData.core.status === LinphoneEnums.CallStatus.Declined || modelData.core.status
																	 ...
																	 === LinphoneEnums.CallStatus.EarlyAborted ? AppIcons.arrowElbow : modelData.core.isOutgoing
														...
														colorizationColor: modelData.core.status === LinphoneEnums.CallStatus.Declined || modelData.core.status
																		   === LinphoneEnums.CallStatus.DeclinedElsewhere || modelData.core.status
																		   === LinphoneEnums.CallStatus.Aborted || modelData.core.status
																		   === LinphoneEnums.CallStatus.EarlyAborted || modelData.core.status
																		   === LinphoneEnums.CallStatus.Missed ? DefaultStyle.danger_500_main : modelData.core.isOutgoing
																												 ? DefaultStyle.info_500_main : DefaultStyle.success_500_main
												...
												Text {
													...
													color: modelData.core.status === LinphoneEnums.CallStatus.Missed ? DefaultStyle.danger_500_main :
