...
AbstractMainPage {
	...
	*/
	...
	onSelectedChatGuiChanged: {
		if (selectedChatGui) {
			if (!listStackView.currentItem || listStackView.currentItem.objectName !== "chatListItem") {
				...
				if (listStackView.depth === 0 || listStackView.currentItem && listStackView.currentItem.objectName
						!== "chatListItem")
					listStackView.push(chatListItem);
			}
		}
	...
	rightPanelStackView.visible:
		false//listStackView.currentItem && listStackView.currentItem.objectName === "chatListItem" && selectedChatGui !== null

	onNoItemButtonPressed: goToNewChat()
	signal newChatItemOpen

	showDefaultItem: listStackView.currentItem && listStackView.currentItem.objectName == "chatListItem"
					 && listStackView.currentItem.listView.count === 0 && !listStackView.currentItem.listView.loading || false

	function goToNewChat() {
		if (listStackView.currentItem && listStackView.currentItem.objectName != "newChatItem")
		...
		GroupCreationFormLayout {
			...
			onGroupCreationRequested: {
				...
				mainWindow.showLoadingPopup(qsTr("chat_creation_in_progress"), true, function () {
					if (chatCreationLayout.groupChat)
						chatCreationLayout.groupChat.core.lDelete();
				});
				chatCreationLayout.groupChatObj = UtilsCpp.createGroupChat(chatCreationLayout.groupName.text,
