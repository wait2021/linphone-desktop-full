...
ApplicationWindow {
	...
	onActiveChanged: {
		if (active)
			UtilsCpp.setLastActiveWindow(this);
	}
	onVisibleChanged: {
		AppCpp.handleAppActivity();
	}
	Component.onDestruction: if (UtilsCpp.getLastActiveWindow() === this)
								 UtilsCpp.setLastActiveWindow(null)
	...
	Component {
		...
		Dialog {
			...
			Component.onCompleted: if (details.length != 0)
									   radius = 0
	...
	function startCallWithContact(contact, videoEnabled, parentItem) {
		if (parentItem == undefined)
			parentItem = mainWindow.contentItem;
		if (contact) {
			...
			if (contact.core.allAddresses.length > 1) {
				...
				var addressToCall = contact.core.defaultAddress.length === 0 ? contact.core.phoneNumbers.length === 0 ? "" :
																														contact.core.phoneNumbers[0].address : contact.core.defaultAddress;
				if (addressToCall.length != 0)
					UtilsCpp.createCall(addressToCall, {
											'localVideoEnabled': videoEnabled
										});
			}
		}
	...
	function transferCallToContact(call, contact, parentItem) {
		...
		if (parentItem == undefined)
			parentItem = mainWindow.contentItem;
		if (contact) {
			console.log("[AbstractWindow] Transfer call to", contact.core.fullName, "addresses count",
						contact.core.allAddresses.length, call);
			if (contact.core.allAddresses.length > 1) {
				var addressPopup = addressChooserPopupComp.createObject();
				addressPopup.parent = parentItem;
				addressPopup.contact = contact;
	...
	function showInformationPopup(title, description, isSuccess) {
		if (isSuccess == undefined)
			isSuccess = true;
		var infoPopup = popupComp.createObject(popupLayout, {
												   "title": title,
	...
	function showLoadingPopup(text, cancelButtonVisible, callback) {
		if (cancelButtonVisible == undefined)
	...
	function showConfirmationPopup(requestDialog) {
		...
		var popup = confirmPopupComp.createObject(popupLayout, {
	...
	function showConfirmationLambdaPopup(title, text, details, callback, firstButtonText, secondButtonText,
		...
		if (firstButtonText != undefined)
			popup.firstButtonText = firstButtonText;
		if (secondButtonText != undefined)
			popup.secondButtonText = secondButtonText;
