...
AbstractWindow {
	...
	function openMainPage(connectionSucceed) {
		if (mainWindowStackView.currentItem.objectName !== "mainPage")
			mainWindowStackView.replace(mainPage, StackView.Immediate);
		//: "Connexion réussie"
		if (connectionSucceed)
			mainWindow.showInformationPopup(qsTr("information_popup_connexion_succeed_title"),
	...
	function initStackViewItem() {
		if (accountProxy && accountProxy.isInitialized) {
			if (accountProxy.haveAccount)
				openMainPage();
			else if (SettingsCpp.getFirstLaunch())
				mainWindowStackView.replace(welcomePage, StackView.Immediate);
			else if (SettingsCpp.assistantGoDirectlyToThirdPartySipAccountLogin)
	...
	function reauthenticateAccount(identity, domain, callback) {
		if (authenticationPopupOpened)
			return;
		if (mainWindowStackView.currentItem.objectName === "loginPage" || mainWindowStackView.currentItem.objectName
				=== "sipLoginPage")
			return;
		console.log("Showing authentication dialog");
		var popup = authenticationPopupComp.createObject(mainWindow, {
															 "identity": identity,
															 "domain": domain,
															 "callback": callback
														 });	// Callback ownership is not passed
		popup.open();
		popup.announce();
	}
	...
	Component {
		...
		LoginPage {
			...
			StackView.onActivated: if (mainWindow.accountProxy?.haveAccount)
									   showBackButton = true
		}
	}
	Component {
		...
		SIPLoginPage {
			...
			onGoToRegister: mainWindowStackView.replace(registerPage)
			showBackButton: false
			StackView.onActivated: if (!SettingsCpp.assistantGoDirectlyToThirdPartySipAccountLogin || mainWindow.accountProxy
										   ?.haveAccount)
									   showBackButton = true
		}
	}
		...
		RegisterPage {
			...
			Connections {
				...
				function onNewAccountCreationSucceed(withEmail, address, sipIdentityAddress) {
					mainWindowStackView.push(checkingPage, {
												 "registerWithEmail": withEmail,
												 "address": address,
												 "sipIdentityAddress": sipIdentityAddress
											 });
				}
				...
				function onTokenConversionSucceed() {
					mainWindow.closeLoadingPopup();
				}
			}
		}
			...
			Connections {
				...
				function onLinkingNewAccountWithCodeFailed(errorMessage) {
					...
					if (errorMessage.length === 0)
						errorMessage = qsTr("assistant_register_error_code");
